...
byte seconds_2m;  // Keeps track of the "wind speed/dir avg" over last 2min data
byte minutes;     // Keeps track of where we are in various arrays of data
byte minutes_10m;  // Keeps track of where we are in wind gust/dir last 10m data
...
volatile float
    rainHour[60];  // 60 floating numbers to keep track of 60 minutes of rain

// These are all the weather values that wunderground expects:
int winddir = 0;         // [0-360 instantaneous wind direction]
float windspeedmph = 0;  // [mph instantaneous wind speed]
float windgustmph =
    0;  // [mph current wind gust, using software specific time period]
...
// float baromin = 30.03;// [barom in] - It's hard to calculate baromin locally,
// do this in the agent
float pressure = 0;
// float dewptf; // [dewpoint F] - It's hard to calculate dewpoint locally, do
// this in the agent
...
// D2
{
  ...
  if (raininterval >
      10)  // ignore switch-bounce glitches less than 10mS after initial edge
  {
...
// input D3
{
  if (millis() - lastWindIRQ >
      10)  // Ignore switch-bounce glitches less than 10ms (142MPH max reading)
...
void httpServer() {
  ...
  while (client.connected()) {
    ...
    if (c == '\n' && currentLineIsBlank) {
      ...
      client.print(windspdmph_avg2m, 1);
      client.print(";winddir_avg2m=");
      client.print(winddir_avg2m);
      client.print(";windgustmph_10m=");
      client.print(windgustmph_10m, 1);
      ...
      client.print(dailyrainin, 2);
      client.print(";pressure=");
      client.print(pressure, 2);
      client.print(";light_lvl=");
      client.print(light_lvl, 2);
      client.println("");
...
void calcWeather() {
  ...
  // Calc windspeed
  // windspeedmph = get_wind_speed(); //This is calculated in the main loop on
  // line 185
  ...
  // You can't just take the average. Google "mean of circular quantities" for
  // more info We will use the Mitsuta method because it doesn't require trig
  // functions And because it sounds cool. Based on:
  // http://abelian.org/vlf/bearings.html Based on:
  // http://stackoverflow.com/questions/1813483/averaging-angles-again
  ...
  rainin = 0;
  for (int i = 0; i < 60; i++) rainin += rainHour[i];
...
int get_wind_direction() {
  ...
  // sorted from low to high. Each threshold is the midpoint between adjacent
  // headings. The output is degrees for that ADC reading. Note that these are
  // not in compass degree order! See Weather Meters datasheet for more
  // information.
...
// I don't like the way this function is written but Arduino doesn't support
// floats under sprintf
void printWeather() {
